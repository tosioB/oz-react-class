...
const increment1 = { // Action
  type: 'increment1'
}

const decrement1 = { // Action
  type: 'decrement1'
}

const increment2 = { // Action
  type: 'increment2'
}

const decrement2 = { // Action
  type: 'decrement2'
}

const counter1Reducer = (state = 0, action) => { // Reducer
  switch (action.type) {
    case 'increment1':
      return state + 1;
    case 'decrement1':
...
const counter2Reducer = (state = 0, action) => { // Reducer
  switch (action.type) {
    case 'increment2':
      return state + 1;
    case 'decrement2':
...
export const store = legacy_createStore(rootReducer, applyMiddleware(thunk)); // Store
// Store를 만들때 middleware를 덧붙여줄 수 있다.
// applyMiddleware를 사용해서 사용할 미들웨어를 연결할 수 있다.
...
function App() {
  const counter1 = useSelector(state => state.counter1Reducer) // useSelector - 상태값을 꺼내와서 사용
  const counter2 = useSelector(state => state.counter2Reducer) // useSelector - 상태값을 꺼내와서 사용
  const dispatch = useDispatch();

  return (
    <>
      <div>Counter1: {counter1}</div>
      <button type="button" className="btn" onClick={() => dispatch((dispatch) => {
        setTimeout(() => {
          dispatch(increment1);
        }, 1000);
      })}>+</button>
...
// Redux
// redux 설치 - npm i redux react-redux
